...
워커     advisor, worker-1, worker-2, worker-3 상태
Advisor  현재 판단 요약
다음     기다릴지, 새 작업을 배정할지
...
버그바운티에서도 APK/AAB/DEX/JAR를 받을 수 있으면 `--file`로 넘기면 됩니다. `huntctl`은
파일을 target workspace의 `input-files/`로 복사하고 worker 컨테이너에서 접근 가능한 경로를
runbook에 기록합니다.

```bash
huntctl start --profile bug-bounty \
  --scope https://app.example.com \
  --file ./target.apk
```

APK가 있으면 scheduler는 `mobile/app-link/API client surface` lane을 먼저 포함합니다. worker는
`apktool`, `jadx`, `aapt`, `apksigner`로 manifest/exported component/deep link/WebView/API host를
확인하고, Docker host에 `/dev/kvm`이 있으면 `android-emulator-headless`, `android-wait-for-boot`,
`adb install`, `adb logcat`, deep link 실행, safe traffic/proxy evidence까지 시도합니다. 에뮬레이터
런타임이 안 되면 어떤 런타임/KVM/계정/프록시 조건이 필요한지 `blocked`로 남깁니다.
...
worker/background Codex 실행은 role별 reasoning effort를 자동으로 붙입니다. 기본 worker는
`worker-1`, `worker-2`, `worker-3`처럼 단순하게 뜨고 `xhigh`로 실행합니다. 직접 role을
지정한 경우 `recon`, `endpoint-mapper`, `file-triage`, `solver`, `validator`, `web-recon`,
`reverse`, `crypto`, `pwn`, `mobile`, `android`, `api`, `exploit`, `fuzz` 계열도 `xhigh`입니다.
`report-writer`/`writeup`은 `medium`, `evidence`는 `high`, advisor는 `low`가 기본입니다.
runbook의 agent에 `reasoning_effort: low|medium|high|xhigh`를 직접 쓰면 그 값이 우선합니다.
...
worker prompt도 작업 내용별로 줄입니다. 기본 `worker-N`은 scope/rules 미리보기, taxonomy 요약,
참조 경로를 받고, 보고서 초안/최종 검토 작업일 때만 report template 원문을 더 자세히 받습니다.
전체 scope, VRT, 보고서 템플릿이 필요하면 prompt에 있는 runbook/reference path를 worker가 직접
필요한 부분만 읽습니다.
...
────────────────────────────────────────
worker-1 [실행중] worker
목표 https://www.example.com / SQLi·server-side injection 후보 확인 중

worker-2 [실행중] worker
목표 https://api.example.com / access-control 후보 재현성 확인 중
```
...
Bug bounty
  ...
  worker     worker-N이 web domain/API/mobile artifact별 focus를 나눠 보고, advisor가 검토와 전환 지시
  전환       2회 연속 새 evidence/capability가 없으면 근거를 남기고 인접 lane으로 이동
  산출물     advisor 검토를 통과한 platform report, PoC/request, evidence, impact, remediation
```
...
기본 scheduler는 VRT/weakness lane을 골고루 돌면서도 web domain이 여러 개면 `worker-1`,
`worker-2`, `worker-3`에 서로 다른 target focus를 배정합니다. 될 것 같은 후보가 생기면 advisor가
해당 후보를 최종 검토하고, 부족한 PoC/증거/impact만 worker에게 다시 보강시킵니다.
...
Bug bounty Decision
  report-ready    advisor 최종 검토를 요청할 만큼 scope, capability, impact, repro, PoC, evidence가 충족
...
blocker를 제거하지 않는 한 다시 배정하지 않습니다. `report-ready`가 있어도 바로 제출용으로 확정하지
않고 advisor가 마지막으로 scope, 공격자 악용 가능성, concrete impact, 재현성, PoC 코드/HTTP 요청,
증거 경로, taxonomy/severity를 확인합니다. 통과하지 못하면 worker에게 누락 증거만 보강시킵니다.
...
huntctl assign worker-1 "https://www.example.com에서 XSS/SQLi 후보를 낮은 요청 수로 확인"
huntctl assign worker-2 "https://api.example.com에서 IDOR/BOLA 후보를 재현 가능성 기준으로 확인"
```
...
```text
advisor           대화/지휘/최종 검토
worker-1          첫 번째 target/domain/lane focus
worker-2          두 번째 target/domain/lane focus
worker-3          세 번째 target/domain/lane focus
```

worker 이름은 단순하지만 작업 내용은 advisor가 나눕니다. 버그바운티에서는 domain/API/mobile
artifact와 VRT lane을 분산하고, CTF에서는 서로 다른 풀이 가설/기법으로 분산합니다.
...
flag가 없을 때 worker는 긴 설명을 만들기보다 실패한 가설, 버린 이유, 다음 worker가
...
huntctl summary
huntctl resume
huntctl logs worker-1
huntctl loop

huntctl scope add https://app.example.com
huntctl scope exclude https://help.example.com
huntctl assign worker-2 "기존 evidence 기준으로 후보를 비파괴적으로 검증"
